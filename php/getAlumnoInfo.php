<?php
   ...
   // Preparamos la query que vamos a ejecutar: Obtenemos la informacion de un grupo seleccionado
   $sentencia->free_result ();
   if (! ($sentencia = $conexion->prepare ("SELECT nombre, apellido1, apellido2 FROM registra WHERE id IN (SELECT id_profesor FROM profes_seleccionados WHERE id_alumno = (?));")))
